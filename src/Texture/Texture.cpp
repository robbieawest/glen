...
#include "Utils/LogType.h"
 ...
 Texture Texture::from_image(std::string filename, Shader& shaderProgram, const std::string& uniform_id) {
	...
	if (spl_res.size() != 2) helper::log(LogType::ERROR, __func__, "Invalid filename format: {}", {filename});
	std::string extension = spl_res[1];
	if (extension != "png" && extension != "jpg") helper::log(LogType::ERROR, __func__, "Invalid file extension: {}", {extension});
	...
	if (unit > 31) helper::log(LogType::ERROR, __func__, "Texture unit cannot be greater than 31");
	...
	if (im_data) {
		...
		helper::log(LogType::ERROR, __func__, "Texture not loaded. File: {}", {filename});
	}

	stbi_image_free(im_data);

	helper::log(LogType::INFO, __func__, "Texture successfully loaded from file {}", {path});
	ret.defined = true;
	return ret;
}
...
Texture* Texture::activate() {
	if (type == 0 || !defined) helper::log(LogType::ERROR, __func__, "Texture activated before definition or type set");
	glc(glActiveTexture(type));
	return this;
}

Texture* Texture::bind() {
	if (!defined) helper::log(LogType::ERROR, __func__, "Texture bound before definition");
